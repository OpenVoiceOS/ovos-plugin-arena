...
config block, everything a user would need. If the same plugin can run two
meaningfully different ways (a different model, a different confidence gate,
a different language), that is **two fighter files**, not one file with a
variants list — each one benches, ranks and appears on the boards separately.
...
- **`langs`** is `["de-DE"]` — a full BCP-47 tag, not `"de"`. The inner
  ...
  are configured with the bare language, some with the full tag — the fighter
  copies whatever the plugin's own config schema wants); `langs` at the
  top level is what the arena uses to route the fighter to eval datasets and
  is always full BCP-47, so it composes with every other fighter's `langs`
  list without ambiguity (`de` alone doesn't tell you `de-DE` vs `de-AT`).
- **`model`** and **`notes`** exist because this is a *neural* fighter with a
  specific model checkpoint — that's bestiary metadata (optional per the
  field table), useful because "Vosk" alone doesn't tell a reader which of
  several published models this fighter actually runs.
- There is exactly **one** `vosk-small-de.json`. The larger German Vosk model
  would be a sibling file (`vosk-big-de.json`, if/when someone adds it), not
  a second entry inside this one — same plugin, different shippable config,
  different fighter.

An intent fighter follows the same rule with a `intents.pipeline` list
instead of a `stt` block — see the intent example in
[`add-a-fighter.md`](add-a-fighter.md#1-write-the-competitor-file). A
multi-stage `pipeline` (e.g. Padatious → Adapt) is still **one** fighter file:
the "shippable config" is the whole cascade, because that's what a user would
actually configure.
...
Both validate every file in `registry/` against the schema — malformed JSON,
a missing required field, or a `modality` that doesn't match the directory
all fail loudly here rather than silently in CI. See
...
- **Intent leagues** run directly: `benchmarks/intent_*.py` reads every
  registered competitor for its modality straight from the registry each time
  it runs (see [`benchmarks.md`](benchmarks.md)) — no separate queueing step,
  just re-run the benchmark script (optionally with `--upload`) and the new
  fighter is included.
- **Audio modalities** (`stt`, `wake_word`, `tts`) go through the always-on
  ...
  --modality stt` diffs the registry against what's already published on
...
1. `assemble.yml` (daily, or `python -m arena.cli assemble` locally — see
   ...
   picks up the new rows next time it runs, adds the fighter to the
   benchmark board and folds it into blind A/B battle pools against every
   other fighter already benched in that league/language.
...
against another fighter in the same league/language to get an ELO rating at

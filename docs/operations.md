...
2. **Hourly tally.** `.github/workflows/tally.yml` runs at :17 past every
   ...
   every `vote`-labelled issue — open *and* closed, full history every time
   — parses titles, deduplicates, applies the anti-fraud rules
...
5. **Rating moves.** The commit tally pushes to `dev` is the rating change
   — there is no separate "publish" step. Once the Pages deploy after that
...
Anyone — not just a maintainer with push access — can reproduce the
current standings from nothing but the public repository:
